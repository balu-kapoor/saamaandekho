...
@section('content')
    ...
    <div class="alert @if($customer->is_kyc_approved) alert-success @else alert-warning @endif d-flex align-items-center justify-content-between border-0" role="alert">
        @if($customer->is_kyc_approved)
            <div>
                <span class="svg-icon">
                    <svg>
                ...
                <span class="ms-2">Your KYC is verified.</span>
            </div>
        @else
            <div>
                <span class="svg-icon">
                ...
                <span class="ms-2">Your KYC is not verified yet.</span>
                <a class="box-shadow" href="{{ url('vendor/kyc') }}" style="font-weight: 800;">Verify Now</a>
            </div>
        @endif
    </div>
