                            ...
                            <div class="vendor-info-content px-3">
                                <div class="vendor-store-information row align-items-center">
                                    ...
                                    <div class="vendor-store-info col">
                                        ...
                                        @php
                                            $vendor = Botble\Ecommerce\Models\Customer::find($store->customer_id);
                                            $addresses = Botble\Ecommerce\Models\Address::where('customer_id', $store->customer_id)
                                            ->orderByDesc('is_default')
                                            ->orderByDesc('created_at')
                                            ->get();
                                        @endphp
                                        @if($vendor && $vendor->is_kyc_approved == 1)
                                        <div class="kyc_badge">
                                            <img src="{{ asset('themes/farmart/images/kyc.png') }}" alt="">
                                        </div>
                                        @endif
                                        @if($addresses->count())
                                        <i class="icon icon-map"></i>&nbsp; 
                                            ...
                                            @php
                                            $userAddress = $addr->address . ', ' . $addr->city . ', ' .$addr->state .', '.$addr->zip_code; // Replace this with the user's address

                                            // Encode the address to make it URL-friendly
                                            $encodedAddress = urlencode($userAddress);

                                            // Construct the Google Maps link
                                            $googleMapsLink = "https://www.google.com/maps?q=" . $encodedAddress;

                                            @endphp
                                           State: {{ $addr->state }}
                                                City: {{ $addr->city }}
                                                Address: {{ $addr->address }}
                                                <a class="btn-primary view-map" href="{{ $googleMapsLink }}" target="_blank">View on map</a>
                                            @endforeach
                                        @endif
                                    </div>
                                </div>
