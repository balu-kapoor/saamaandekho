...
class ProductController extends BaseController
{
    ...
    {
        if(!auth('customer')->user()->addresses()->count()) {
            return redirect()->to('customer/overview');
        }
    ...
    public function featureProduct(int|string $id, BaseHttpResponse $response)
    {
        $product = Product::query()->findOrFail($id);

        PageTitle::setTitle(__('Feature product'));

        $vendorId = auth('customer')->user()->id;
        $vendor = Customer::find($vendorId);

        return MarketplaceHelper::view('dashboard.feature', compact(['vendor']));
    }
